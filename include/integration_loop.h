...
namespace dealii
{
  ...
  void integration_loop (const std::vector<ITERATOR>                         &cell_range,
                         MeshWorker::DoFInfo< dim, spacedim >                &dof_info,
                         MeshWorker::IntegrationInfoBox< dim, spacedim >     &info_box,
                         const MeshWorker::LocalIntegrator< dim, spacedim >  &integrator,
                         ASSEMBLER                                           &assembler,
                         const MeshWorker::LoopControl                       &lctrl = MeshWorker::LoopControl())
  {
    ...
    MeshWorker::DoFInfoBox<dim, MeshWorker::DoFInfo<dim, spacedim> > dof_info_box(dof_info);
    ...
    for (unsigned int i=0; i<cell_range.size(); ++i)
      {
        cell_action<MeshWorker::IntegrationInfoBox<dim, spacedim>,
                    MeshWorker::DoFInfo<dim, spacedim>, dim, spacedim >
                    (cell_range[i],cell_range, dof_info_box, info_box, cell_worker,
